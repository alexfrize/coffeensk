...
export default class SendMessageForm extends React.Component {
  ...
  updateCheckoutPossibility() {
    this.canCheckout =
      ...
      this.state.customerMessage.length > 10;
  }
    ...
    fetch('/php/send-message.php', {
      ...
      .then(response => {
        ...
        if (response.status === 200 || response.status === 0) {
          ...
          alert(
            'При отправке сообщения возникла ошибка. Пожалуйста, повторите попытку чуть позже, либо свяжитесь с нами по телефону.'
          );
        }
      })
      .catch(error => {
    ...
    return (
      <div className="col-md-12">
        ...
        <form className="send-message__form" onSubmit={this.handleSubmit}>
          ...
          />
          <textarea
            className="send-message__form__textarea"
          ...
          <div className="send-message__send-message-button-container">
            <button className="send-message__send-message-button">
              Отправить сообщение
            </button>
          </div>
        </form>
